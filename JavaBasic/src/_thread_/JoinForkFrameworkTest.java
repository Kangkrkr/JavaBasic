...
public class JoinForkFrameworkTest {
	...
	private static class SumOfSquaresTask extends RecursiveTask<Long> {
		...
		// forkJoin.invoke 시, compute() 메소드가 자동으로 실행된다.
		@Override
		protected Long compute() {
			long sum = 0L;
			int mid = (to + from) >>> 1;	// 양의 값으로 반으로 줄인다.
			...
			if ((to - from) < threshold) {
				...
				// 중간값을 기준으로 반으로 나누어, 자기자신들을 다시 호출한다.
